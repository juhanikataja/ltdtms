...
export lbox, ltanhstep, lpost, ThrData, lpost_joint, lpost_simple
...
function hit_simple(s, MT, EE, K, smax, n) # {{{
  exponent = 0
  for m = 1:length(MT)
    exponent = exponent - ((MT[m]*sum(EE[m,:,n].*s)-1))^2 / (2*K^2)
    exponent = exponent + log(abs(sum(EE[m,:,n].*s)))
    exponent = exponent - log(sqrt(2*pi)*K) 
  end
  return exponent
end # }}}
